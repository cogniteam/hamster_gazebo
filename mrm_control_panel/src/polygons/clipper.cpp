...
double Area(const PolyTree &poly){
	double asum=0;
	NodesIterator ni(poly);
	for(size_t i=0;i<ni.size();i++){
		if(ni[i]->IsHole()==false) asum+=Area(ni[i]->Contour);
	}
	return asum;
}
...
Clipper::Clipper() : ClipperBase() //constructor
{
  ...
  m_ForceSimple = false;
  m_SubjFillType = pftNonZero;
  m_ClipFillType = pftNonZero;
...
void _print(std::ostream& cout, std::string tab, const PolyNode* poly){
	using namespace std;
	cout<<tab<<(poly->IsHole()?"HOLE":"OUTTER")<<endl;
	std::stringstream s; s<<poly->Contour;
	string ss = s.str();
	//cout<<tab<<"---{"<<endl;
	cout<<tab;
	...
	cout<<endl;
	//cout<<tab<<"}---{{"<<endl;
	for(size_t i=0;i<poly->Childs.size();i++)
		_print(cout, tab+"   ",poly->Childs[i]);
	//cout<<tab<<"}}---"<<endl;
}
void print(std::ostream& cout, std::string title, const PolyTree& poly){
	...
	NodesIterator ni(poly);
	for(size_t i=0;i<ni.size();i++)
		if(&poly==ni[i]->Parent) _print(cout, "",ni[i]);
	cout<<"==================="<<endl;
}


bool operator<(const IntPoint& p, const Polygon& poly){
	Clipper c; c.AddPolygon(poly, ptSubject);c.AddPolygon(poly, ptClip);
	Polygons res;
	c.Execute(ctUnion, res, pftNonZero, pftNonZero);
	return poly::PointInPolygon(p, c.getFistOutPt(),true);
}
bool operator<(const Polygon& p1, const Polygon& p2){
	Clipper c; c.AddPolygon(p1, ptSubject);c.AddPolygon(p2, ptClip);
	Polygons res;
	c.Execute(ctDifference, res, pftNonZero, pftNonZero);
	return res.empty();
}
void PolygonsToPolyTree(const Polygons& ps, PolyTree& tree){
	Clipper c; c.AddPolygons(ps, ptSubject);c.AddPolygons(ps, ptClip);
	c.Execute(ctUnion, tree, pftNonZero, pftNonZero);
}
void copy(const PolyTree& source, Polygons& dist){
	PolyTreeToPolygons(source, dist);
}
void copy(const Polygons& source, PolyTree& dist){
	PolygonsToPolyTree(source, dist);
}
void copy(const Polygons& source, Polygons& dist){
	dist = source;
}
void copy(const PolyTree& source, PolyTree& dist){
	Polygons tmp; copy(source,tmp); copy(tmp, dist);
}

void _contains(const IntPoint& p, const PolyNode* poly, bool& ins, bool& res){
	bool inside, result;
	ins = p < poly->Contour;
	if(!ins){res=false; return;}
	for(size_t i=0;i<poly->Childs.size();i++){
		_contains(p, poly->Childs[i], inside, result);
		if(inside){ res = result; return; }
	}
	res = !poly->IsHole();
}
...
bool operator<(const IntPoint& p, const PolyTree& poly){
	bool inside, result;
	NodesIterator ni(poly);
	for(size_t i=0;i<ni.size();i++)if(&poly==ni[i]->Parent){
		_contains(p, ni[i], inside, result);
...
void calc_intersection(const Polygons& p1, const Polygons& p2, Polygons& res){
	...
	c.AddPolygons(p1, ptSubject);
	c.AddPolygons(p2, ptClip);
	c.Execute(ctIntersection, res, pftNonZero, pftNonZero);
}
...
void calc_xor(const Polygons& p1, const Polygons& p2, Polygons& res){
	...
	c.AddPolygons(p1, ptSubject);
	c.AddPolygons(p2, ptClip);
	c.Execute(ctXor, res, pftNonZero, pftNonZero);
}
